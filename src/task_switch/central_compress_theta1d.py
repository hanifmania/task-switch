...
from task_switch.field import Field, ObserveField


class CentralXZTheta:
    def __init__(self):
        rospy.init_node("central", anonymous=True)
        # field_param = rospy.get_param("/field")
        # self.field = Field(field_param)
        self._observe = ObserveField(rospy.get_param("/observe"))
    ...
    def observedAreaUpdate(self):
        ...
        Z = self._observe.getPhi()
        q = self._observe.getProjected()
        dists = [np.abs(pos[0] - q) for pos in self.allPositions]
    ...
    def observe2field(self):
        observed = self._observe.getPhi()

    def main(self):
        self.observedAreaUpdate()
        self.observe2field()
