...
from task_switch.pointcloud_theta1d import PointCloudXTheta


class PointCloudXZTheta(PointCloudXTheta):
    """
    ...
    def __init__(self, Hz=100):
        ...
        self.X, self.Y, self.Z = self.field.getGrid()
    ...
    def Float32MultiArrayCallback(self, msg_data):
        ...
        info = np.array(msg_data.data).reshape(
            (
                msg_data.layout.dim[0].size,
                msg_data.layout.dim[1].size,
                msg_data.layout.dim[2].size,
            )
        )
