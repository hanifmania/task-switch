...
class PointCloudOriginShow(object):
    """
    ...
    def __init__(self, Hz=100):
        ...
        field_param = rospy.get_param("/theta_field")
        ...
        x,y = self.q2p((self.X, self.Y))
        x += rospy.get_param("~center_x")
        y += rospy.get_param("~center_y")
        ...
        # ROS set up
        self.pub_pointcloud = rospy.Publisher("pointcloud_theta", PointCloud2, queue_size=1)
        # self.marker_pub = rospy.Publisher("field_visualization", Marker, queue_size=1)
        self.surf_value_sub = rospy.Subscriber(
            "theta_phi", Float32MultiArray, self.Float32MultiArrayCallback
        )

        rospy.wait_for_message("theta_phi", Float32MultiArray)
        # param initialize
