...
class Voronoi(object):
    ...
    def calcVoronoi(self):
        self.Region = (self.X - self.Pos[0]) ** 2 + (
            self.Y - self.Pos[1]
        ) ** 2 < self.R ** 2
        # create R+margin circle
        outside = (self.X - self.Pos[0]) ** 2 + (self.Y - self.Pos[1]) ** 2 < (
            1.1 * self.R
        ) ** 2

        # calculate distance to each point in R+margin circle
        distance = (self.X * outside - self.Pos[0]) ** 2 + (
            self.Y * outside - self.Pos[1]
        ) ** 2
        ...
        for neighborPos in self.listNeighborPos:
            ...
            neighborDistance = (self.X * self.Region - neighborPos[0]) ** 2 + (
                self.Y * self.Region - neighborPos[1]
            ) ** 2
        ...
        self.cent = (
            np.array([weightedX.sum(), weightedY.sum()]) * self.pointDense / self.mass
        )
        ...
        expandX = (
            vectorX[vectorNorm > 0]
            / vectorNorm[vectorNorm > 0]
            * onEdgePhi[vectorNorm > 0]
        )
        expandY = (
            vectorY[vectorNorm > 0]
            / vectorNorm[vectorNorm > 0]
            * onEdgePhi[vectorNorm > 0]
        )
        self.expand = (
            (self.R ** 2 + self.b)
            * np.array([expandX.sum(), expandY.sum()])
            * self.pointDense
        )
    ...
    def getdJdp(self):
        return 2 * self.mass * (self.Pos[0] - self.cent[0])
            ...
            for i in range(AgentNum):
                ...
                pos[i] = (
                    ...
                    + (
                        -pos[i]
                        + Agents[i].getCent()
                        - Agents[i].getExpand() / (2 * Agents[i].getMass())
                    )
                    * 0.2
                )
